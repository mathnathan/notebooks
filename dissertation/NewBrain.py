...
class Neuron():
    ...
    def __init__(self, shape, weights, bias, decay=0.025, pi=1):
        ...
        self.const = np.power(2*np.pi, self.dim)
    ...
    def __call__(self, x, feedback=1, update=True):
        ...
        if update: # Can only update batches of size 1 currently
            ...
            c = np.sqrt(self.const*self.bias)
            self.weights = self.weights + self.lr*output*z.sum(axis=0)
            self.bias = self.bias + self.lr*(np.maximum(output*(z_dot_z-self.bias),-0.2*self.bias) + self.decay*c*self.bias)
            self.lr = np.maximum(0,self.lr-0.0075)
